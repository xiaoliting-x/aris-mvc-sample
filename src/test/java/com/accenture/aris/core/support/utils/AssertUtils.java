...
public class AssertUtils {
    ...
    public static Map<String, List<?>> getData(String xml, String xls) throws FileNotFoundException, InvalidFormatException, IOException, ParserConfigurationException, SAXException{
        Map<String, List<?>> map = getMap(AssertUtils.class.getClassLoader().getResourceAsStream(xml));
        XLSReader reader = ReaderBuilder.buildFromXML(AssertUtils.class.getClassLoader().getResourceAsStream(xml));
        reader.read(AssertUtils.class.getClassLoader().getResourceAsStream(xls), map);
        return map;
    }
